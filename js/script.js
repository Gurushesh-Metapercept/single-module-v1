...
document.addEventListener("DOMContentLoaded", () => {
  ...
  currenthref = currenthref.split("#")[0].split("?")[0];  
    ...
    let currentIndex = -1;
    for (let i = 0; i < aTagsList.length; i++) {
      if (aTagsList[i].href === currenthref) {
        currentIndex = i;
        break;
      }
    }
      ...
      if (parentElement && parentElement.tagName === "LI") {
        ...
        if (previousSibling) {
          ...
          if (index === currentIndex) {
            if(aTagsList[index - 1] !== undefined){
              let elseArrLen = aTagsList[index - 1].innerText

              const dot = elseArrLen.length > 30 ? "..." : ""
 
              previousBtn.innerHTML = `
           <a href="${aTagsList[index - 1].href}" class="previous d-flex">
           <div class="arrowIcon me-3 d-flex justify-content-center align-items-center">
             <div class="d-flex justify-content-center align-items-center">
               <svg width="20" height="20" fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M10 13h8V7h-8V2l-8 8 8 8v-5z"/></svg>
             </div>
        ...
        nextBtn.innerHTML = `
          ...
          if (index === currentIndex) {
            if(aTagsList[index + 1] !== undefined){
            let arrLen = aTagsList[index + 1].innerText
            const dot = arrLen.length > 30 ? "..." : "" 
              
            nextBtn.innerHTML = `
                <a href="${aTagsList[index + 1].href}" class="next d-flex">   
